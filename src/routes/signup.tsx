...
function SignupPage() {
  ...
  async function handleGoogleSignUp() {
    ...
    const result = await authClient.signIn.social({
      ...
      callbackURL: "/",
      errorCallbackURL:
        "/signup?error=Google%20sign-up%20could%20not%20be%20completed.",
    })

    if (result.error) {
      setError(result.error.message ?? "Google sign-up could not be completed.")
        ...
        <section className="flex items-center justify-center px-4 py-10 sm:px-8 lg:px-12 lg:py-16">
          <Card className="w-full max-w-[520px] gap-0 rounded-[28px] border-white/80 bg-white py-0 shadow-[0_16px_48px_rgba(52,35,80,0.10)]">
            <CardHeader className="gap-0 px-6 pb-7 pt-8 sm:px-10 sm:pt-10">
              ...
              <p className="mb-3 text-sm font-semibold text-primary">Get started</p>
              ...
              <CardDescription className="mt-3 max-w-md text-base leading-6">
                Create your free account and bring your movement and health signals together.
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="px-6 pb-8 sm:px-10 sm:pb-10">
              ...
              >
                <form.AppField name="name">
                  {(field) => (
                    <field.TextField label="Your name" />
                  )}
                </form.AppField>
                <form.AppField
                  name="email"
